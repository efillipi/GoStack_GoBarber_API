...
class UpdateProfileService {
  ...
  public async execute({ user_id, name, email, old_password, password }: IRequest): Promise<User> {
    ...
    if (password && old_password) {
      const user_password = await this.hashProvider.comparreHash(old_password, user.password)

      if (!user_password) {
        throw new AppError('Operação não Autozirada, senha anterior informada incorreta', 401)
      }
